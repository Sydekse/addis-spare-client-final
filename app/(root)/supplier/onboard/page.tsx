...
import { createSupplier } from "@/lib/api/services/supplier.service";
...
export default function Page() {
  ...
  const handleSave = async () => {
    ...
    try {
      const newUser = await createSupplier(user.id, formData);
      console.log("Created supplier user:", newUser);
  ...
  const validateForm = (): boolean => {
    return !!(
      ...
      (formData.uploadedFiles.length > 0 || formData.existingFiles?.length)
    );
  };
